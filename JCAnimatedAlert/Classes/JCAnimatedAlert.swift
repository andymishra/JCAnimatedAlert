...
//  AnimatedAlert
//
//  Created by Anand Mishra on 26/08/18.
//  Copyright © 2018 JustClickApps. All rights reserved.
//
...
open class JCAnimatedAlert: UIViewController {
    ...
    var strongSelf:JCAnimatedAlert?
    var contentView = UIView()
    var titleLabel: UILabel = UILabel()
    var buttons: [UIButton] = []
    var animatedView: JCAnimatableView?
    ...
    fileprivate func resizeAndRelayout() {
        ...
        for i in 0 ..< buttons.count {
                ...
                buttons[i].addTarget(self, action: #selector(JCAnimatedAlert.pressed(_:)), for: UIControlEvents.touchUpInside)
    ...
    func closeAlert(_ buttonIndex:Int){
        if userAction !=  nil {
            ...
            AnimatedAlertContext.shouldNotAnimate = true
            userAction!(isOtherButton)
            AnimatedAlertContext.shouldNotAnimate = false
        }
    ...
    open func showAlert(_ title: String) -> JCAnimatedAlert {
        _ =  showAlert(title, subTitle: nil, style: .none)
        return self
    }
    
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle) -> JCAnimatedAlert {
        _ = showAlert(title, subTitle: subTitle, style: style, buttonTitle: "OK")
        return self

    }

    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> JCAnimatedAlert {
        _ = showAlert(title, subTitle: subTitle, style: style, buttonTitle: buttonTitle,buttonColor: UIColor.colorFromRGB(0xAEDEF4))
        userAction = action
        return self
    }
    
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,action: ((_ isOtherButton: Bool) -> Void)? = nil) -> JCAnimatedAlert {
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        String?, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> JCAnimatedAlert {
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        String?, otherButtonColor: UIColor?,action: ((_ isOtherButton: Bool) -> Void)? = nil) {
            ...
            if otherButtonTitle != nil && otherButtonTitle!.isEmpty == false {
                ...
                button.addTarget(self, action: #selector(JCAnimatedAlert.pressed(_:)), for: UIControlEvents.touchUpInside)
                button.tag = 1
                buttons.append(button)
            }

            resizeAndRelayout()
            if AnimatedAlertContext.shouldNotAnimate == true {
    ...
    fileprivate struct AnimatedAlertContext {
...
class JCAnimatableView: UIView {
    func animate(){
    }
}

class CancelAnimatedView: JCAnimatableView {
...
class InfoAnimatedView: JCAnimatableView {
...
class SuccessAnimatedView: JCAnimatableView {
